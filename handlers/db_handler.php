...
class DbHandler implements DbHandlerInterface
{
    ...
    */
    ...
    {
        global $dbCon;
        $insert_query = "INSERT INTO account_requests(first_name,last_name,business_name,business_description,region_id,specific_location,cat_id,email,subbed) VALUES(?,?,?,?,?,?,?,?,?)";

        //Attempt to prepare query
        if($insert_stmt = $dbCon->prepare($insert_query))
        {
            $insert_stmt->bind_param("ssssisisi",$details["first_name"],$details["last_name"],$details["business_name"],$details["business_description"],$details["region_id"],$details["specific_location"],$details["cat_id"],$details["email"],$details["subbed"]);
